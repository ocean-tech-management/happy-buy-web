...
{{--        <a class="btn btn-sm btn-info" href="{{ route('admin.' . $crudRoutePart . '.to-ship', ['id' => $row->id]) }}">--}}
{{--            {{ trans('global.to_ship') }}--}}
{{--        </a>--}}
{{--    @endif--}}
{{--@endcan--}}

{{--@can($toPickUpGate)--}}
{{--    @if($row->status == 1 && $row->collect_type == 1)--}}
{{--        <form action="{{ route('admin.' . $crudRoutePart . '.to-pick-up') }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">--}}
{{--            <input type="hidden" name="_token" value="{{ csrf_token() }}">--}}
