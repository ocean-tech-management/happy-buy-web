...
@can($editGate)
    ...
    @else
        <a class="btn btn-sm btn-info" href="{{ route('admin.' . $crudRoutePart . '.package-edit', $row->id) }}">
            {{ trans('global.edit') }}
        </a>
    @endif

@endcan
@can($createVariantGate)
    <a class="btn btn-sm btn-success" href="{{ route('admin.product-variants.create', $row->id) }}">
        {{ trans('global.add_variant') }}
    </a>
@endcan
@can($deleteGate)
