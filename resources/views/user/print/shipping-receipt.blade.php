    ...
    <div class="card">
        <div class="card-header p-4">
            ...
            <div class="float-right">
                <h3 class="mb-0">Receipt #{{ $receipt->receipt_number }}</h3>
                Date: {{ $receipt->updated_at->format('d/m/Y') }}
            </div>
        </div>
        <div class="card-body">
                    ...
                    <tr>
                        <td style="width: 50%">
                            <div class="text-dark">
                                ...
                                @if($receipt->from_name && $receipt->from_name != null)
                                <h3 class="text-dark mb-1">{{ $receipt->from_name }}</h3>
                                <div>Email: {{ $receipt->from_email }}</div>
                                <div>Phone: {{ $receipt->from_phone }}</div>
                                @else
            ...
            <div class="table-responsive-sm">
                <table class="table table-striped">
                    ...
                    <tr>
                        ...
                        <td class="left strong">{{ "Package ".$receipt->shipping_package->point }}</td>
                        <td class="left">Shipping Package with value {{ $receipt->shipping_package->point }} PV</td>
                        <td class="right">RM {{ number_format($receipt->shipping_package->price,2) }}</td>
                        <td class="center">1</td>
                        <td class="right">RM {{ number_format($receipt->shipping_package->price,2) }}</td>
                    </tr>
                ...
                <div class="col-lg-4 col-sm-5 ml-auto">
                    <table class="table table-clear">
                        ...
                        <tr>
                            ...
                            <td class="right">
                                <strong class="text-dark">RM {{ number_format($receipt->shipping_package->price,2) }}</strong>
                            </td>
                        </tr>
                        </tbody>
                    </table>
...
{{--            <p class="mb-0"><span>Extra Notes: </span>  {{ $receipt->footnote }} </p>--}}
{{--        </div>--}}
    </div>
</div>
