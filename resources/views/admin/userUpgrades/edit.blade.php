...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.userUpgrade.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-upgrades.update", [$userUpgrade->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $userUpgrade->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('upgrade_role') ? 'is-invalid' : '' }}" name="upgrade_role_id" id="upgrade_role_id" required>
                    @foreach($upgrade_roles as $id => $entry)
                        <option value="{{ $id }}" {{ (old('upgrade_role_id') ? old('upgrade_role_id') : $userUpgrade->upgrade_role->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount') ? 'is-invalid' : '' }}" type="text" name="amount" id="amount" value="{{ old('amount', $userUpgrade->amount) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" name="payment_method_id" id="payment_method_id">
                    @foreach($payment_methods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('payment_method_id') ? old('payment_method_id') : $userUpgrade->payment_method->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                    ...
                    @foreach(App\Models\UserUpgrade::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $userUpgrade->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('gateway_response') ? 'is-invalid' : '' }}" name="gateway_response" id="gateway_response">{{ old('gateway_response', $userUpgrade->gateway_response) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gateway_status') ? 'is-invalid' : '' }}" name="gateway_status" id="gateway_status">
                    ...
                    @foreach(App\Models\UserUpgrade::GATEWAY_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gateway_status', $userUpgrade->gateway_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gateway_transaction') ? 'is-invalid' : '' }}" type="text" name="gateway_transaction" id="gateway_transaction" value="{{ old('gateway_transaction', $userUpgrade->gateway_transaction) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('approve_at') ? 'is-invalid' : '' }}" type="text" name="approve_at" id="approve_at" value="{{ old('approve_at', $userUpgrade->approve_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('approved_by_user') ? 'is-invalid' : '' }}" name="approved_by_user_id" id="approved_by_user_id">
                    @foreach($approved_by_users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('approved_by_user_id') ? old('approved_by_user_id') : $userUpgrade->approved_by_user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('approved_by_admin') ? 'is-invalid' : '' }}" name="approved_by_admin_id" id="approved_by_admin_id">
                    @foreach($approved_by_admins as $id => $entry)
                        <option value="{{ $id }}" {{ (old('approved_by_admin_id') ? old('approved_by_admin_id') : $userUpgrade->approved_by_admin->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
