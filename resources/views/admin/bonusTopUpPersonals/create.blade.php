...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.bonusTopUpPersonal.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.bonus-top-up-personals.store") }}" enctype="multipart/form-data">
                @csrf
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('point_package') ? 'is-invalid' : '' }}" name="point_package_id" id="point_package_id" required>
                        @foreach($point_packages as $id => $entry)
                            <option value="{{ $id }}" {{ old('point_package_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
