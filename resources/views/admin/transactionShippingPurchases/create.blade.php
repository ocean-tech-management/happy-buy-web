...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.transactionShippingPurchase.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.transaction-shipping-purchases.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                <label class="required" for="user_id">{{ trans('cruds.transactionShippingPurchase.fields.user') }}</label>
            ...
            <div class="form-group">
                <label class="required" for="shipping_package_id">{{ trans('cruds.transactionShippingPurchase.fields.shipping_package') }}</label>
                <select class="form-control select2 {{ $errors->has('shipping_package') ? 'is-invalid' : '' }}" name="shipping_package_id" id="shipping_package_id" required>
                    @foreach($shipping_packages as $id => $entry)
                        <option value="{{ $id }}" {{ old('shipping_package_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label class="required" for="payment_method_id">{{ trans('cruds.transactionShippingPurchase.fields.payment_method') }}</label>
            ...
            <div class="form-group">
                <label class="required" for="receipt">{{ trans('cruds.transactionShippingPurchase.fields.receipt') }}</label>
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control {{ $errors->has('gateway_status') ? 'is-invalid' : '' }}" name="gateway_status" id="gateway_status">
                    ...
                    @foreach(App\Models\TransactionShippingPurchase::GATEWAY_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gateway_status', '0') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
