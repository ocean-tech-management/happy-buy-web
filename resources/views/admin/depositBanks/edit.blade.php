...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.depositBank.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.deposit-banks.update", [$depositBank->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('bank') ? 'is-invalid' : '' }}" name="bank_id" id="bank_id" required>
                    @foreach($banks as $id => $entry)
                        <option value="{{ $id }}" {{ (old('bank_id') ? old('bank_id') : $depositBank->bank->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('bank'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bank_account_name') ? 'is-invalid' : '' }}" type="text" name="bank_account_name" id="bank_account_name" value="{{ old('bank_account_name', $depositBank->bank_account_name) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('bank_account_number') ? 'is-invalid' : '' }}" type="number" name="bank_account_number" id="bank_account_number" value="{{ old('bank_account_number', $depositBank->bank_account_number) }}" step="1" required>
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\DepositBank::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $depositBank->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
