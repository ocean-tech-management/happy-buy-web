...
{{--    @endcan--}}
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.report.fields.deposit_balance') }} {{ trans('global.list') }}
        </div>

        <form action="{{ route('admin.reports.deposit-balance.export') }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <div class="card-body">
            <div class="row px-5 mb-1 d-flex mt-5">
                {{-- <div class="flex-grow-1 input-div col-2">
                    <input type="text" class="form-control datatable-input" name="date" id="date" onfocus="(this.type='date')" onblur="(this.type='text')" placeholder="{{trans('global.enter_for', ['value'=>trans('global.date')])}}">
                </div> --}}
        ...
        <div class="card-body">
            <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-UserEntry">
                ...
                <tr>
                    ...
                    <th>
                        {{ trans('cruds.report.fields.user_id') }}
                    </th>
                    <th>
                        {{ trans('cruds.report.fields.user_name') }}
                    </th>
                    <th>
                        {{ trans('cruds.report.fields.amount') }}
                    </th>
                    <th>
                        {{ trans('cruds.report.fields.total') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
    ...
    <script>
        $(function () {
            ...
            let dtOverrideGlobals = {
                ...
                paging: false,
                ajax: {
                    url: "{{ route('admin.reports.deposit-balance') }}",
                    data: function (d) {
                        // d.date = $('#date').val();
                        d.user = $('#user').val();
