                    ...
                    <div class="col-md-6">
                        <table class="table table-responsive table-bordered table-hover">
                            <tbody>
                                ...
                                @if(count($bonus_credit) == 1)
                                <tr>
                                    <th>{{ trans('cruds.report.fields.transaction_period') }}</th>
                                    <td field-key="transaction_period">{{\Carbon\Carbon::parse($bonus_credit[0]->transaction_date)->format('Y-m-d')}}</td>
                                </tr>
                                @elseif(count($bonus_credit) >= 2)
                                <tr>
                                    <th>{{ trans('cruds.report.fields.transaction_period') }}</th>
                                    <td field-key="transaction_period">{{\Carbon\Carbon::parse($bonus_credit[0]->transaction_date)->format('Y-m-d')}} - {{\Carbon\Carbon::parse($bonus_credit[count($bonus_credit)-1]->transaction_date)->format('Y-m-d')}}</td>
                                </tr>
                                @endif
                            </tbody>
                        </table>
                ...
                <div class="row pt-3">
                    <div class="col-md-12">
                        <table class="table table-responsive table-bordered table-hover">
                            ...
                            <tbody>
                                <?php $total = 0; ?>
                                @if(count($bonus_credit) != 0)
                                    @foreach($bonus_credit as $key => $item)
                                        <?php 
                                        switch($item->type) {
                                                case "1":
                                                    $total += $item->amount;
