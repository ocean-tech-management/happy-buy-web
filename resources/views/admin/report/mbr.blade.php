...
{{--    @endcan--}}
    <div class="card">
        <div class="card-header">
            {{ trans('cruds.report.fields.mbr') }} {{ trans('global.list') }}
        </div>

        <form action="{{ route('admin.reports.mbr.export') }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
            ...
            <div class="row px-5 mb-1 d-flex mt-5">
                <h4 name="total_amount" id="total_amount"></h4>
                <h4 name="total_quantity" id="total_quantity"></h4>
            </div>
        </div>
        </form>
        ...
        <div class="card-body">
            <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-UserEntry">
                ...
                <tr>
                    ...
                    <th>
                        {{ trans('cruds.report.fields.document_no') }}
                    </th>
                    <th>
                    ...
                    <th>
                        {{ trans('cruds.userEntry.fields.order_id') }}
                    </th>
                    <th>
                        {{ trans('cruds.userEntry.fields.amount') }}
                    </th>
                    <th>
                        {{ trans('cruds.userEntry.fields.quantity') }}
                    </th>
                </tr>
                </thead>
            </table>
                ...
                ajax: {
                    ...
                    "dataSrc": function (data)
                    {
                        if($('#start_date').val() != '' && $('#end_date').val() != '') {
                            $('#total_amount').html($.fn.dataTable.render.number(',', '.', 2, '').display("Filtered Total Amount: "  + data.totalAmount));
                            $('#total_quantity').html($.fn.dataTable.render.number(',', '.', 2, '').display("Filtered Total Quantity: "  + data.totalQuantity));
                        }
                        return data.data;
                    },
                },
                columns: [
                    { data: 'id', name: 'document_mbr_invoice_logs.id' , visible: false},
