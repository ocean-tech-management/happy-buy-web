...
@section('content')
@can('point_manager_balance_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.point-manager-balances.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.pointManagerBalance.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.pointManagerBalance.title_singular') }} {{ trans('global.list') }}
    </div>

    <form action="{{ route('admin.point-manager-balances.export') }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
        ...
        <div class="card-body">
            <div class="row px-5 mb-1 d-flex mt-5">
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    ...
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.pointManagerBalance.fields.user')])}}">
                </div>

                <div class="flex-grow-1 input-div col-lg-3">
                    <input type="text" class="form-control datatable-input" id="amount" name="amount" 
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.pointManagerBalance.fields.amount')])}}">
                </div>

                <div class="flex-grow-1 input-div col-lg-3">
                    <select class="form-control datatable-input" id="status" name="status">
                        <option value ="">{{trans('global.select_for', ['value'=>trans('cruds.pointManagerBalance.fields.status')])}}</option>
                        @foreach(App\Models\PointManagerBalance::STATUS_SELECT as $key => $item)
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    <select class="form-control datatable-input" id="settlement" name="settlement">
                        <option value ="">{{trans('global.select_for', ['value'=>trans('cruds.pointManagerBalance.fields.settlement')])}}</option>
                        @foreach(App\Models\PointManagerBalance::SETTLEMENT_SELECT as $key => $item)
                            <option value="{{$key}}">{{$item}}</option>>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    ...
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.pointManagerBalance.fields.remark')])}}">
                </div>
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-PointManagerBalance">
            <thead>
                <tr>
{{--                    <th width="10">--}}
...
@can('point_manager_balance_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.point-manager-balances.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    aaSorting: [],
    ajax: {
        url:"{{ route('admin.point-manager-balances.index') }}",
        data:function(d){
            d.user = $('#user').val();
            d.amount = $('#amount').val();
            d.status = $('#status').val(); 
  ...
  let table = $('.datatable-PointManagerBalance').DataTable(dtOverrideGlobals);
