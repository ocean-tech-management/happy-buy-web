...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.transactionPointPurchase.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.transaction-point-purchases.update", [$transactionPointPurchase->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="transaction">{{ trans('cruds.transactionPointPurchase.fields.transaction') }}</label>
                <input class="form-control {{ $errors->has('transaction') ? 'is-invalid' : '' }}" type="text" name="transaction" id="transaction" value="{{ old('transaction', $transactionPointPurchase->transaction) }}">
            ...
            <div class="form-group">
                <label for="user_id">{{ trans('cruds.transactionPointPurchase.fields.user') }}</label>
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $transactionPointPurchase->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                <label for="point_package_id">{{ trans('cruds.transactionPointPurchase.fields.point_package') }}</label>
                <select class="form-control select2 {{ $errors->has('point_package') ? 'is-invalid' : '' }}" name="point_package_id" id="point_package_id">
                    @foreach($point_packages as $id => $entry)
                        <option value="{{ $id }}" {{ (old('point_package_id') ? old('point_package_id') : $transactionPointPurchase->point_package->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('point') ? 'is-invalid' : '' }}" type="number" name="point" id="point" value="{{ old('point', $transactionPointPurchase->point) }}" step="0.01">
                @if($errors->has('point'))
                    <div class="invalid-feedback">
                        {{ $errors->first('point') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $transactionPointPurchase->price) }}" step="0.01">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                <label for="payment_method_id">{{ trans('cruds.transactionPointPurchase.fields.payment_method') }}</label>
                <select class="form-control select2 {{ $errors->has('payment_method') ? 'is-invalid' : '' }}" name="payment_method_id" id="payment_method_id">
                    @foreach($payment_methods as $id => $entry)
                        <option value="{{ $id }}" {{ (old('payment_method_id') ? old('payment_method_id') : $transactionPointPurchase->payment_method->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\TransactionPointPurchase::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $transactionPointPurchase->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <label for="receipt">{{ trans('cruds.transactionPointPurchase.fields.receipt') }}</label>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('payment_verified_at') ? 'is-invalid' : '' }}" type="text" name="payment_verified_at" id="payment_verified_at" value="{{ old('payment_verified_at', $transactionPointPurchase->payment_verified_at) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('admin') ? 'is-invalid' : '' }}" name="admin_id" id="admin_id">
                    @foreach($admins as $id => $entry)
                        <option value="{{ $id }}" {{ (old('admin_id') ? old('admin_id') : $transactionPointPurchase->admin->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('admin'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('gateway_response') ? 'is-invalid' : '' }}" name="gateway_response" id="gateway_response">{{ old('gateway_response', $transactionPointPurchase->gateway_response) }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('gateway_status') ? 'is-invalid' : '' }}" name="gateway_status" id="gateway_status">
                    ...
                    @foreach(App\Models\TransactionPointPurchase::GATEWAY_STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('gateway_status', $transactionPointPurchase->gateway_status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('gateway_transaction') ? 'is-invalid' : '' }}" type="text" name="gateway_transaction" id="gateway_transaction" value="{{ old('gateway_transaction', $transactionPointPurchase->gateway_transaction) }}">
