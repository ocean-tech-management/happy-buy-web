...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pointTransactionLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.point-transaction-logs.update", [$pointTransactionLog->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $pointTransactionLog->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $pointTransactionLog->date) }}" required>
                @if($errors->has('date'))
                    <div class="invalid-feedback">
                        {{ $errors->first('date') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('top_up') ? 'is-invalid' : '' }}" type="text" name="top_up" id="top_up" value="{{ old('top_up', $pointTransactionLog->top_up) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('point_convert') ? 'is-invalid' : '' }}" type="text" name="point_convert" id="point_convert" value="{{ old('point_convert', $pointTransactionLog->point_convert) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('redemption') ? 'is-invalid' : '' }}" type="text" name="redemption" id="redemption" value="{{ old('redemption', $pointTransactionLog->redemption) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('shipping') ? 'is-invalid' : '' }}" type="text" name="shipping" id="shipping" value="{{ old('shipping', $pointTransactionLog->shipping) }}" required>
