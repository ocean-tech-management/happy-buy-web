...
@section('content')
@can('voucher_balance_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.voucher-balances.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.voucherBalance.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.voucherBalance.title_singular') }} {{ trans('global.list') }}
    </div>

    <form id="form" action="{{ route('admin.voucher-balances.export') }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
        ...
        <div class="card-body">
            <div class="row px-5 mb-1 d-flex mt-5">
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    ...
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.voucherBalance.fields.user')])}}">
                </div>

                <div class="flex-grow-1 input-div col-lg-3">
                    <input type="text" class="form-control datatable-input" id="amount" name="amount" 
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.voucherBalance.fields.amount')])}}">
                </div>

                <div class="flex-grow-1 input-div col-lg-3">
                    <select class="form-control datatable-input" id="status" name="status">
                        <option value ="">{{trans('global.select_for', ['value'=>trans('cruds.voucherBalance.fields.status')])}}</option>
                        @foreach(App\Models\VoucherBalance::STATUS_SELECT as $key => $item)
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    <select class="form-control datatable-input" id="settlement" name="settlement">
                        <option value ="">{{trans('global.select_for', ['value'=>trans('cruds.voucherBalance.fields.settlement')])}}</option>
                        @foreach(App\Models\VoucherBalance::SETTLEMENT_SELECT as $key => $item)
                            <option value="{{$key}}">{{$item}}</option>>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    ...
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.voucherBalance.fields.remark')])}}">
                </div>
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-VoucherBalance">
            <thead>
                <tr>
{{--                    <th width="10">--}}
...
@can('voucher_balance_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.voucher-balances.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    aaSorting: [],
    ajax: {
        url:"{{  route('admin.voucher-balances.index') }}",
        data:function(d){
            d.user = $('#user').val();
            d.amount = $('#amount').val();
            d.status = $('#status').val(); 
...
{ data: 'actions', name: '{{ trans('global.actions') }}' }
  ...
  let table = $('.datatable-VoucherBalance').DataTable(dtOverrideGlobals);
