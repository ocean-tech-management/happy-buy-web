...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.transactionRedeemProduct.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.transaction-redeem-products.update", [$transactionRedeemProduct->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group" hidden>
                <label for="transaction">{{ trans('cruds.transactionRedeemProduct.fields.transaction') }}</label>
                <input class="form-control {{ $errors->has('transaction') ? 'is-invalid' : '' }}" type="text" name="transaction" id="transaction" value="{{ old('transaction', $transactionRedeemProduct->transaction) }}">
            ...
            <div class="form-group"  hidden>
                ...
                <select class="form-control select2 {{ $errors->has('product') ? 'is-invalid' : '' }}" name="product_id" id="product_id" required>
                    @foreach($products as $id => $entry)
                        <option value="{{ $id }}" {{ (old('product_id') ? old('product_id') : $transactionRedeemProduct->product->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('variant') ? 'is-invalid' : '' }}" name="variant_id" id="variant_id" required>
                    @foreach($variants as $id => $entry)
                        <option value="{{ $id }}" {{ (old('variant_id') ? old('variant_id') : $transactionRedeemProduct->variant->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $transactionRedeemProduct->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control {{ $errors->has('purchase_price') ? 'is-invalid' : '' }}" type="number" name="purchase_price" id="purchase_price" value="{{ old('purchase_price', $transactionRedeemProduct->purchase_price) }}" step="0.01">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control {{ $errors->has('purchase_quantity') ? 'is-invalid' : '' }}" type="text" name="purchase_quantity" id="purchase_quantity" value="{{ old('purchase_quantity', $transactionRedeemProduct->purchase_quantity) }}">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control {{ $errors->has('pre_point_balance') ? 'is-invalid' : '' }}" type="number" name="pre_point_balance" id="pre_point_balance" value="{{ old('pre_point_balance', $transactionRedeemProduct->pre_point_balance) }}" step="0.01">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control {{ $errors->has('post_point_balance') ? 'is-invalid' : '' }}" type="text" name="post_point_balance" id="post_point_balance" value="{{ old('post_point_balance', $transactionRedeemProduct->post_point_balance) }}">
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\TransactionRedeemProduct::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $transactionRedeemProduct->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('collect_type') ? 'is-invalid' : '' }}" name="collect_type" id="collect_type">
                    ...
                    @foreach(App\Models\TransactionRedeemProduct::COLLECT_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('collect_type', $transactionRedeemProduct->collect_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address_id" id="address_id">
                    @foreach($addresses as $id => $entry)
                        <option value="{{ $id }}" {{ (old('address_id') ? old('address_id') : $transactionRedeemProduct->address->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control select2 {{ $errors->has('shipped_by') ? 'is-invalid' : '' }}" name="shipped_by_id" id="shipped_by_id">
                    @foreach($shipped_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('shipped_by_id') ? old('shipped_by_id') : $transactionRedeemProduct->shipped_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control select2 {{ $errors->has('completed_by') ? 'is-invalid' : '' }}" name="completed_by_id" id="completed_by_id">
                    @foreach($completed_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('completed_by_id') ? old('completed_by_id') : $transactionRedeemProduct->completed_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control select2 {{ $errors->has('refund_by') ? 'is-invalid' : '' }}" name="refund_by_id" id="refund_by_id">
                    @foreach($refund_bies as $id => $entry)
                        <option value="{{ $id }}" {{ (old('refund_by_id') ? old('refund_by_id') : $transactionRedeemProduct->refund_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control {{ $errors->has('tracking_code') ? 'is-invalid' : '' }}" type="text" name="tracking_code" id="tracking_code" value="{{ old('tracking_code', $transactionRedeemProduct->tracking_code) }}">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control datetime {{ $errors->has('refund_at') ? 'is-invalid' : '' }}" type="text" name="refund_at" id="refund_at" value="{{ old('refund_at', $transactionRedeemProduct->refund_at) }}">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control datetime {{ $errors->has('pickup_at') ? 'is-invalid' : '' }}" type="text" name="pickup_at" id="pickup_at" value="{{ old('pickup_at', $transactionRedeemProduct->pickup_at) }}">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control datetime {{ $errors->has('shipout_at') ? 'is-invalid' : '' }}" type="text" name="shipout_at" id="shipout_at" value="{{ old('shipout_at', $transactionRedeemProduct->shipout_at) }}">
            ...
            <div class="form-group" hidden>
                ...
                <input class="form-control datetime {{ $errors->has('completed_at') ? 'is-invalid' : '' }}" type="text" name="completed_at" id="completed_at" value="{{ old('completed_at', $transactionRedeemProduct->completed_at) }}">
