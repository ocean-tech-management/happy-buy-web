...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.productQuantity.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.product-quantities.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sold_to_user') ? 'is-invalid' : '' }}" name="sold_to_user_id" id="sold_to_user_id">
                    @foreach($sold_to_users as $id => $entry)
                        <option value="{{ $id }}" {{ old('sold_to_user_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('qr_generate_at') ? 'is-invalid' : '' }}" type="text" name="qr_generate_at" id="qr_generate_at" value="{{ old('qr_generate_at') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('in_stock_at') ? 'is-invalid' : '' }}" type="text" name="in_stock_at" id="in_stock_at" value="{{ old('in_stock_at') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('sold_at') ? 'is-invalid' : '' }}" type="text" name="sold_at" id="sold_at" value="{{ old('sold_at') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('first_scan_at') ? 'is-invalid' : '' }}" type="text" name="first_scan_at" id="first_scan_at" value="{{ old('first_scan_at') }}">
