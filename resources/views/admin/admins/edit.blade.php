            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route("admin.admins.update", [$admin->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <h4 class="card-title mb-3">{{ trans('global.edit') }} {{ trans('cruds.admin.title_singular') }}</h4>
                        <div class="row">
                            ...
                            <div class="form-group col-6">
                                ...
                                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $admin->phone) }}">
                                @if($errors->has('phone'))
                                    <div class="invalid-feedback">
                                        {{ $errors->first('phone') }}
                                    </div>
                            ...
                            <div class="form-group col-6" hidden>
                                ...
                                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                                    ...
                                    @foreach(App\Models\Admin::STATUS_SELECT as $key => $label)
                                        <option value="{{ $key }}" {{ old('status', $admin->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                    @endforeach
                                </select>
