...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.shippingCompany.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.shipping-companies.update", [$shippingCompany->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.shippingCompany.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $shippingCompany->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('api_name') ? 'is-invalid' : '' }}" type="text" name="api_name" id="api_name" value="{{ old('api_name', $shippingCompany->api_name) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\ShippingCompany::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $shippingCompany->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
