...
@section('content')
@can('point_balance_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.point-balances.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.pointBalance.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.pointBalance.title_singular') }} {{ trans('global.list') }}
    </div>

    <form action="{{ route('admin.point-balances.export') }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
        ...
        <div class="card-body">
            <div class="row px-5 mb-1 d-flex mt-5">
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    ...
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.pointBalance.fields.user')])}}">
                </div>

                <div class="flex-grow-1 input-div col-lg-3">
                    <input type="text" class="form-control datatable-input" id="amount" name="amount" 
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.pointBalance.fields.amount')])}}">
                </div>

                <div class="flex-grow-1 input-div col-lg-3">
                    <select class="form-control datatable-input" id="status" name="status">
                        <option value ="">{{trans('global.select_for', ['value'=>trans('cruds.pointBalance.fields.status')])}}</option>
                        @foreach(App\Models\PointBalance::STATUS_SELECT as $key => $item)
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    <select class="form-control datatable-input" id="settlement" name="settlement">
                        <option value ="">{{trans('global.select_for', ['value'=>trans('cruds.pointBalance.fields.settlement')])}}</option>
                        @foreach(App\Models\PointBalance::SETTLEMENT_SELECT as $key => $item)
                            <option value="{{$key}}">{{$item}}</option>>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="flex-grow-1 input-div col-lg-3">
                    ...
                    placeholder ="{{trans('global.enter_for', ['value'=>trans('cruds.pointBalance.fields.remark')])}}">
                </div>
    ...
    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-PointBalance">
            <thead>
                <tr>
{{--                    <th width="10">--}}
...
{{--                    </th>--}}
                    ...
                    <th>
                        {{ trans('cruds.pointBalance.fields.remark') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
...
@can('point_balance_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.point-balances.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    aaSorting: [],
    ajax: {
        url:"{{ route('admin.point-balances.index') }}",
        data:function(d){
            d.user = $('#user').val();
            d.amount = $('#amount').val();
            d.status = $('#status').val(); 
  ...
  let table = $('.datatable-PointBalance').DataTable(dtOverrideGlobals);
