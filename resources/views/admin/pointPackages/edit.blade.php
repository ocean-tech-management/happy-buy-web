...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pointPackage.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.point-packages.update", [$pointPackage->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="row">
                <div class="form-group col-6">
                    <label class="required" for="name_en">{{ trans('cruds.pointPackage.fields.name_en') }}</label>
                    <input class="form-control {{ $errors->has('name_en') ? 'is-invalid' : '' }}" type="text" name="name_en" id="name_en" value="{{ old('name_en', $pointPackage->name_en) }}" required>
                ...
                <div class="form-group col-6">
                    ...
                    <input class="form-control {{ $errors->has('name_zh') ? 'is-invalid' : '' }}" type="text" name="name_zh" id="name_zh" value="{{ old('name_zh', $pointPackage->name_zh) }}">
                ...
                <div class="form-group col-6">
                    ...
                    <input class="form-control {{ $errors->has('point') ? 'is-invalid' : '' }}" type="number" name="point" id="point" value="{{ old('point', $pointPackage->point) }}" step="0.01">
                    @if($errors->has('point'))
                        <div class="invalid-feedback">
                            {{ $errors->first('point') }}
                        </div>
                ...
                <div class="form-group col-6">
                    ...
                    <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" value="{{ old('price', $pointPackage->price) }}" step="0.01">
                    @if($errors->has('price'))
                        <div class="invalid-feedback">
                            {{ $errors->first('price') }}
                        </div>
                ...
                <div class="form-group col-6">
                    ...
                    <input class="form-control {{ $errors->has('deduct_point') ? 'is-invalid' : '' }}" type="text" name="deduct_point" id="deduct_point" value="{{ old('deduct_point', $pointPackage->deduct_point) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('deduct_2_level_point') ? 'is-invalid' : '' }}" type="text" name="deduct_2_level_point" id="deduct_2_level_point" value="{{ old('deduct_2_level_point', $pointPackage->deduct_2_level_point) }}">
                ...
                <div class="form-group col-6">
                    ...
                    <select class="form-control select2 {{ $errors->has('roles') ? 'is-invalid' : '' }}" name="roles[]" id="roles" multiple>
                        @foreach($roles as $id => $role)
                            <option value="{{ $id }}" {{ (in_array($id, old('roles', [])) || $pointPackage->roles->contains($id)) ? 'selected' : '' }}>{{ $role }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('roles'))
                        <div class="invalid-feedback">
            ...
            <div class="form-group" hidden>
                ...
                <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status">
                    ...
                    @foreach(App\Models\PointPackage::STATUS_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('status', $pointPackage->status) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
