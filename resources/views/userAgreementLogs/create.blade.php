...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.userAgreementLog.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.user-agreement-logs.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user_agreement') ? 'is-invalid' : '' }}" name="user_agreement_id" id="user_agreement_id" required>
                    @foreach($user_agreements as $id => $entry)
                        <option value="{{ $id }}" {{ old('user_agreement_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('signature_name') ? 'is-invalid' : '' }}" type="text" name="signature_name" id="signature_name" value="{{ old('signature_name', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('signature_ic') ? 'is-invalid' : '' }}" type="text" name="signature_ic" id="signature_ic" value="{{ old('signature_ic', '') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('signature_at') ? 'is-invalid' : '' }}" type="text" name="signature_at" id="signature_at" value="{{ old('signature_at') }}" required>
