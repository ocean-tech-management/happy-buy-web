...
class LanguageController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $table->editColumn('actions', function ($row) {
                ...
                $statusChangeGate = 'language_status_change';
                $crudRoutePart = 'languages';

                return view('partials.datatablesActions_Language', compact(
                'viewGate',
                'editGate',
                'deleteGate',
                'statusChangeGate',
            ...
            $table->editColumn('status', function ($row) {
                return isset($row->status) ? Language::STATUS_SELECT[$row->status] : '';
            });
    ...
    {
        $model = Language::findOrFail(request('id'));
        $model->update([
