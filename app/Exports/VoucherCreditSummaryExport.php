...
class VoucherCreditSummaryExport implements FromQuery, WithMapping, WithHeadings, WithCustomStartCell, WithColumnFormatting, WithColumnWidths, WithStyles, ShouldQueue
{
    ...
    {
        return ReportVoucherCredit::with(['user'])->search($this->request)->select(sprintf('*', (new ReportVoucherCredit())->table));
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if($this->request->start_date == null) {
            $result = ReportVoucherCredit::with(['user'])->search($this->request)->select(sprintf('*', (new ReportVoucherCredit())->table))->first();
            if($result) {
        ...
        $sheet->setCellValue('C2', 'VOUCHER CREDIT REPORT (SUMMARY)');
