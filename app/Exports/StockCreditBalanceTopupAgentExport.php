...
class StockCreditBalanceTopupAgentExport implements FromCollection, WithMapping, WithHeadings, WithCustomStartCell, WithColumnFormatting, WithColumnWidths, WithStyles, ShouldQueue
{
    ...
    public function collection()
    {
        ...
        if($this->request->has('start_date') && filled($this->request['start_date'])) {
            ...
            $filtered = $query->filter(function ($item) use ($pbQuery) {
                $pbData = $pbQuery->get();
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->setCellValue('C2', 'STOCK CREDIT REPORT (BALANCE TOP UP AGENT)');
