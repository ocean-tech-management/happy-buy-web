...
class BonusCreditBalanceExport implements FromQuery, WithMapping, WithHeadings, WithCustomStartCell, WithColumnFormatting, WithColumnWidths, WithStyles, ShouldQueue
{
    ...
    {
        return ReportBonusCredit::with(['user'])->search($this->request)->groupBy('user_id')
        ->select(
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if($this->request->start_date == null) {
            $result = ReportBonusCredit::with(['user'])->search($this->request)->groupBy('user_id')
            ->select(
                sprintf('*', (new ReportBonusCredit())->table),
                DB::raw('SUM(report_bonus_credits.amount) AS personalAmount'),
            )->first();
        ...
        $sheet->setCellValue('C2', 'BONUS CREDIT REPORT (BALANCE)');
