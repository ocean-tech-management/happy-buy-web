...
class StockCreditBalanceExport implements FromCollection, WithMapping, WithHeadings, WithCustomStartCell, WithColumnFormatting, WithColumnWidths, WithStyles, ShouldQueue
{
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if($this->request->start_date == null) {
            $result = ReportStockCredit::with(['user'])->search($this->request)->groupBy('user_id')
            ->whereHas('user', fn ($query) => 
        ...
        $sheet->setCellValue('C2', 'STOCK CREDIT REPORT (BALANCE)');
