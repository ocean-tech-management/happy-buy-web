...
class VoucherCreditBalanceExport implements FromQuery, WithMapping, WithHeadings, WithCustomStartCell, WithColumnFormatting, WithColumnWidths, WithStyles, ShouldQueue
{
    ...
    {
        return ReportVoucherCredit::with(['user'])->search($this->request)->groupBy('user_id')
        ->select(
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if($this->request->start_date == null) {
            $result = ReportVoucherCredit::with(['user'])->search($this->request)->groupBy('user_id')
            ->select(
                sprintf('*', (new ReportVoucherCredit())->table),
                DB::raw('SUM(report_voucher_credits.amount) AS personalAmount'),
            )->first();
        ...
        $sheet->setCellValue('C2', 'VOUCHER CREDIT REPORT (BALANCE)');
