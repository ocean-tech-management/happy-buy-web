...
class ShippingCreditSummaryExport implements FromQuery, WithMapping, WithHeadings, WithCustomStartCell, WithColumnFormatting, WithColumnWidths, WithStyles, ShouldQueue
{
    ...
    {
        return ReportShippingCredit::with(['user'])->search($this->request)->select(sprintf('*', (new ReportShippingCredit())->table));
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        if($this->request->start_date == null) {
            $result = ReportShippingCredit::with(['user'])->search($this->request)->select(sprintf('*', (new ReportShippingCredit())->table))->first();
            if($result) {
        ...
        $sheet->setCellValue('C2', 'SHIPPING CREDIT REPORT (SUMMARY)');
