...
class StockCreditBalanceTopupAgentDetailExport implements FromCollection, WithMapping, WithHeadings, WithCustomStartCell, WithColumnFormatting, WithColumnWidths, WithStyles, ShouldQueue
{
    ...
    public function collection()
    {
        ...
        $order = Order::where('user_id', $this->request['user_id'])
        ->whereIn('status', [1,2,3])->whereIn('wallet_type', [1,2])
        ->whereBetween('created_at', [$this->request->start_date != null ? $this->request->start_date : $this->default_start_date, $this->request->end_date != null ? $this->request->end_date : Carbon::now()])
        ->select(
            'id AS from_table_id',
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        $sheet->setCellValue('B2', 'STOCK CREDIT REPORT (BALANCE TOP UP AGENT DETAIL)');
