...
class DocumentInvoiceLog extends Model
{
    use SoftDeletes;
    use Auditable;
    use HasFactory;

    public $table = 'document_invoice_logs';
    ...
    public function scopeGenerateDocumentNumber($query, $user_id, $from_user_id = null, $year = null)
    {
        ...
        $count = DocumentInvoiceLog::count();
        ...
        do {
            ...
            $check = DocumentInvoiceLog::where('name', $key)->count();
        } while ($check >= 1);

        DocumentInvoiceLog::create(['name' => $key, 'user_id' => $user_id, 'from_user_id' => $from_user_id]);
